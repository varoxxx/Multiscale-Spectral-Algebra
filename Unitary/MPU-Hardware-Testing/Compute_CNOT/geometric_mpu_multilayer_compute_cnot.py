...
def build_function_unitary(N=2):
    dim = 2**N
    U = cp.zeros((dim, dim), dtype=cp.complex128)
    for x in range(dim):
        y = reversible_function_2bit_simple(x)
        U[y, x] = 1.0
    return U


def reversible_function_2bit_simple(x):
    """
CNOT
    """
    mapping = {
...
def simulate_function_check(qc_decomp, N=2, num_samples=4):
    """
    Simulate the MPU circuit on basis states and verify it implements the reversible function.
    """
    print("\n=== SIMULATION FUNCTION CHECK (NO HARDWARE) ===")
    backend = AerSimulator()

    for x in range(num_samples):
        b_in = format(x, f"0{N}b")
        y_expected = reversible_function_2bit_simple(x)
        b_expected = format(y_expected, f"0{N}b")

        # prepare input |b_in>
        qc_input = qc_decomp.copy()
        qc_input.data = []  # clear existing gates
        # initialize basis state via X gates
        qc_input.reset(range(N))
        for i, bit in enumerate(reversed(b_in)):  # Qiskit uses little-endian
            if bit == "1":
                qc_input.x(i)

        # append MPU unitary
        U_np = np.array(U_mpu_multi.get())
        op = Operator(U_np)
        instr = op.to_instruction()
        qc_input.append(instr, range(N))

        qc_input.measure_all()

        # run simulation
        result = backend.run(qc_input, shots=2048).result()
        counts = result.get_counts()

        # most probable output
        b_out = max(counts.items(), key=lambda kv: kv[1])[0]

        print(f"Input  |{b_in}>  →  expected |{b_expected}>  |  observed top: |{b_out}>")
...
if __name__ == "__main__":
    ...
    # Build target reversible function unitary U_f
    U_multi = build_function_unitary(N=N)
    
    # Project onto MPU manifold
    U_mpu_multi = mpu_projection(U_multi, N, chi)
    U_mpu_multi = align_global_phase(U_multi, U_mpu_multi)
    
    print(f"Operator Frobenius ||U_f - U_mpu||_F = {float(cp.linalg.norm(U_multi - U_mpu_multi)):.6e}")
    print(f"Operator fidelity F(U_mpu, U_f) = {FidelityObjective.operator_fidelity(U_mpu_multi, U_multi):.6f}")
    ...
    #######################################################    
    # SIMULATION FUNCTION CHECK (no hardware)
    simulate_function_check(qc_decomp, N=N, num_samples=4)
    
    # Save the decomposed MPU circuit once (without input preparation)
    qc_base = qc_decomp.copy()
    qc_base.measure_all()
    ...
    service = QiskitRuntimeService(
        channel="ibm_cloud",
        token="X", #INSERT YOUR API KEY HERE
        instance="YOUR_TEST"
    )
    ...
    for label, ops in input_states.items():
    
        print(f"\n=== Preparing input state: {label} ===")
    
        # Start from decomposed MPU circuit (without measurements)
        qc_input = qc_decomp.copy()
        ...
        # 6) SAVE JSON (FULLY JSON-SAFE) FOR THIS INPUT
        ...
        fname = f"ibm_run_multilayer_N2_{label}.json"
        with open(fname, "w") as f:
            json.dump(output_json, f, indent=2)
    
        print(f"\nSaved Multilayer JSON: {fname}")
